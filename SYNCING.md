...
| `ios/Runner.xcodeproj/project.pbxproj` | bundle ID + MPVKitAM URL + app-icon name + signing team | Keep `com.amaze.vibestream`, `MazeDev7/MPVKitAM`, `ASSETCATALOG_COMPILER_APPICON_NAME = vibe_stream` (icon folder `vibe_stream.icon`, NOT `plezy.icon`), and **`DEVELOPMENT_TEAM = MRF7ZX8DD9`** (3×, the Runner target's Debug/Profile/Release). Take Plezy's other settings/file refs. ⚠️ Upstream's pbxproj hard-codes Plezy's team `DEVELOPMENT_TEAM = G88U5B5783`; a sync that takes it will break iPhone device signing (Xcode can't provision under Plezy's team). After any pbxproj merge run `sed -i '' 's/DEVELOPMENT_TEAM = G88U5B5783;/DEVELOPMENT_TEAM = MRF7ZX8DD9;/g'`. The Flutter CLI auto-substitutes the right team so `flutter build` masks this — only Xcode/`xcodebuild` device builds surface it. |
| `macos/Runner.xcodeproj/project.pbxproj` | same (no `DEVELOPMENT_TEAM` set for macOS) | same |
| `ios/vibe_stream.icon`, `macos/vibe_stream.icon` | renamed app-icon folder (Icon Composer) | The fork renamed Plezy's `plezy.icon` → `vibe_stream.icon` (Vibe artwork) and set `ASSETCATALOG_COMPILER_APPICON_NAME = vibe_stream` in both pbxproj. **Zero conflict by design**: upstream's `plezy.icon` edits arrive as clean new files — **discard them** (`git rm -r ios/plezy.icon macos/plezy.icon` if a merge recreates them). Source artwork: `assets/vibe_stream.*`. |
| `android/.../res/{mipmap,drawable}-*/ic_launcher*`, `…/ic_stat_notification*`, `windows/runner/resources/app_icon.ico` | platform app icons at fixed paths | Vibe artwork. These sit at canonical paths upstream also maintains and **can't be renamed**, so expect occasional binary conflicts when Plezy changes their icon — resolve **keep ours** (`git checkout --ours -- <file>`). The fork dropped Plezy's vector `drawable/ic_launcher_foreground.xml` + `ic_launcher_monochrome.xml` for PNG foreground/monochrome (`drawable-*/ic_launcher_foreground.png`, `mipmap-*/ic_launcher_monochrome.png`) with an inset adaptive `mipmap-anydpi-v26/ic_launcher.xml`. Regenerate the `.ico` from `assets/vibe_stream.png` (Pillow: 16–256px multi-size). Linux icons regenerate from `assets/vibe_stream.png` at build time (nothing committed). |
