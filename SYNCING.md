...
| `ios/Runner.xcodeproj/project.pbxproj` | bundle ID + MPVKitAM URL + app-icon name | Keep `com.amaze.vibestream`, `MazeDev7/MPVKitAM`, and `ASSETCATALOG_COMPILER_APPICON_NAME = vibe_stream` (icon folder `vibe_stream.icon`, NOT `plezy.icon`). Take Plezy's other settings/file refs. |
