...
class DownloadProvider extends ChangeNotifier with DisposableChangeNotifierMixin {
  ...
  Future<List<SyncRuleResult>> _executeSyncRulesRaw(MultiServerManager serverManager, {bool force = false}) async {
    ...
    if (_syncRuleCleanupInProgress) return const [];
  ...
  Future<SyncReconcileSummary?> reconcileNow(MultiServerManager serverManager, {String? activeGlobalKey}) async {
    if (!beginReconcile()) return null;
    try {
      return await syncAllRules(serverManager, activeGlobalKey: activeGlobalKey, force: true);
    } finally {
      endReconcile();
    }
  }
