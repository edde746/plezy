...
class PlayerNative extends PlayerBase {
  ...
  Future<void> open(
    ...
    // AVFoundation's renderer-owned volume is unavailable until the audio
    // output exists. Gate initial playback only when that fallback path has a
    // logical volume waiting to be restored.
    ...
    try {
      ...
      // macOS AVFoundation fallback also opens paused so its per-renderer
      // volume can be restored before queued audio becomes audible.
  ...
  Future<void> play() async {
    ...
    if (_usesMacOSOutputVolume && logicalVolume != null && _macOSOutputVolumeReady) {
  ...
  Future<void> _refreshMacOSOutputVolume() async {
    ...
    try {
      ...
      appLogger.w('$logPrefix: failed to restore macOS output volume', error: error, stackTrace: stackTrace);
    }
  }
