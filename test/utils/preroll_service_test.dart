...
void main() {
  ...
  group('prerollShouldPlayFor', () {
    ...
    test('plays for a fresh movie', () {
      ...
      expect(prerollShouldPlayFor(movie, skipPreroll: false, isOffline: false, usePushReplacement: false), isTrue);
    });

    test('does not play for an episode', () {
      final episode = testMediaItem(kind: MediaKind.episode);
      expect(prerollShouldPlayFor(episode, skipPreroll: false, isOffline: false, usePushReplacement: false), isFalse);
    });
    ...
    test('does not play when offline', () {
      final movie = testMediaItem(kind: MediaKind.movie);
      expect(prerollShouldPlayFor(movie, skipPreroll: false, isOffline: true, usePushReplacement: false), isFalse);
    });

    test('does not play when the preroll flag itself requests skipping it', () {
      final movie = testMediaItem(kind: MediaKind.movie);
      expect(prerollShouldPlayFor(movie, skipPreroll: true, isOffline: false, usePushReplacement: false), isFalse);
    });
    ...
    test('does not play when the pref is off, regardless of other conditions', () async {
      ...
      final movie = testMediaItem(kind: MediaKind.movie);
      expect(prerollShouldPlayFor(movie, skipPreroll: false, isOffline: false, usePushReplacement: false), isFalse);
    });
  });
}
