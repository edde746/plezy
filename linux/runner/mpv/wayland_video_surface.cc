...
bool WaylandVideoSurface::BindGlobals(GdkDisplay* display, std::string* error) {
  ...
  // owned by a member, so Destroy() would not see them, and falling back to the
  // texture path is an ordinary outcome rather than a fatal one - a compositor
  // without wl_subcompositor takes this route on every launch.
...
void WaylandVideoSurface::ArmTransitionWatchdog() {
  ...
  watchdog_source_ = g_timeout_add_seconds(
      ...
      +[](gpointer data) -> gboolean {
        ...
        // reaching here at all means the compositor never answered.
        if (self->transition_staged_) {
          g_warning(
...
bool WaylandVideoSurface::CommitHdrTransition(uint64_t token) {
  ...
  // Belt and braces: SettleTransition already cancelled the watchdog on the way
  // here.
  CancelTransitionWatchdog();
...
void WaylandVideoSurface::SetRect(int32_t x, int32_t y, int32_t width, int32_t height, int32_t scale) {
  ...
  // The buffer size must be an integer multiple of the buffer scale, or
  // wl_surface.commit raises the fatal invalid_size error and the compositor
  // disconnects us - the process dies with nothing in our own logs. Both values
  // land on the same commit (the eglSwapBuffers in Present), so a mismatch is
  // caught on the very next presented frame. Round up rather than down: rounding
  // down would shrink the plane below the rect Flutter cut out for it and show
  // the desktop through the gap.
  //
  // Clamped first, because the round-up adds to the value: width arrives as an
  // int32 cast of an unvalidated channel argument, and near INT32_MAX the
  // addition is signed overflow whose negative product would reach
  // wl_egl_window_resize.
  const int32_t block = scale;
  const int32_t ceiling = std::numeric_limits<int32_t>::max() - (block - 1);
  if (width > ceiling) width = ceiling;
  if (height > ceiling) height = ceiling;
  width = width < block ? block : ((width + block - 1) / block) * block;
  height = height < block ? block : ((height + block - 1) / block) * block;
  ...
  auto floor_div = [](int32_t value, int32_t divisor) -> int32_t {
    const int32_t quotient = value / divisor;
    return (value % divisor != 0 && value < 0) ? quotient - 1 : quotient;
  };
  wl_subsurface_set_position(subsurface_, floor_div(x_, scale_), floor_div(y_, scale_));
  RequestParentCommit();
}
