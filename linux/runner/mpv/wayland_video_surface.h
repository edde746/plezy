...
class WaylandVideoSurface {
 ...
 private:
  ...
  // Bounds the wait for the compositor's verdict on a staged image description.
  // Present() and the plugin's render path are both held meanwhile, so silence
  // would otherwise freeze the plane for good.
